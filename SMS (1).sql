...
EXEC sp_rename 'Feedefaulter.feedef_id', 'fees_defaulter_id', 'COLUMN';
ALTER TABLE departments ADD description TEXT;

--10
ALTER TABLE exams ADD duration INT;
ALTER TABLE sdepartments ADD previousnumbers INT;
ALTER TABLE Students alter column last_name VARCHAR(100);
EXEC sp_rename 'departments.department_name', 'dept_name', 'COLUMN';
...
EXEC sp_rename 'Teachers.teacher_id', 'instructor_id', 'COLUMN';
...
EXEC sp_rename 'Subjects.subject_name', 'course_name', 'COLUMN';
...
EXEC sp_rename 'departments.department_id', 'dept_id', 'COLUMN';
...
ALTER TABLE Feedefaulter alter column amountdue DECIMAL;
EXEC sp_rename 'Grades.grade_id', 'grade_record_id', 'COLUMN';
ALTER TABLE salary DROP COLUMN payment_date;
ALTER TABLE exams ADD max_marks INT;
ALTER TABLE grades ADD grade_date DATE;
ALTER TABLE Students alter column last_name VARCHAR(100);
EXEC sp_rename 'Exams.exam_id', 'exam_record_id', 'COLUMN';
ALTER TABLE sdepartments DROP COLUMN sdepartment_name;
ALTER TABLE Subjects ADD semester VARCHAR(20);
ALTER TABLE Feedefaulter alter column amountdue DECIMAL;
--40
EXEC sp_rename 'Attendance.attendance_id', 'attendance_record_id', 'COLUMN';
ALTER TABLE attendance DROP COLUMN date;
ALTER TABLE Teachers ADD joining_date DATE;
ALTER TABLE fees ADD payment_mode VARCHAR(50);
ALTER TABLE salary alter column amount DECIMAL;
EXEC sp_rename 'Enrollments.enrollment_id', 'enrollment_record_id', 'COLUMN';
ALTER TABLE exams DROP COLUMN exam_date;
ALTER TABLE grades ADD grade_percentage DECIMAL;
ALTER TABLE fees alter column amount decimal;
EXEC sp_rename 'StudentDepartments.sdepartment_name', 'student_department_name', 'COLUMN';
--50
...
---(9) ORDER BY using AND, OR and NOT Operators 
SELECT *
FROM Students
...
---(11)-GROUP BY using AND, OR, NOT Operators and Group by 
...
WHERE subject_id NOT IN (SELECT subject_id FROM Enrollments)
      AND credithour > 3;
-----(13) subquries with logical operators
----1 Retrieve students who are enrolled in a specific subject:
SELECT *
FROM Students
WHERE student_id IN (SELECT student_id FROM Enrollments WHERE subject_id = 1);
--2 Retrieve students who are enrolled in more than one subject:
SELECT *
FROM Students
WHERE student_id IN (SELECT student_id FROM Enrollments GROUP BY student_id HAVING COUNT(*) > 1);
--3 Retrieve subjects with credit hours greater than a certain value:
SELECT *
FROM Students
WHERE student_id NOT IN (SELECT student_id FROM Enrollments);
--4 retrieves all the subjects that have a credit hour value less than or equal to 3
SELECT *
FROM Subjects
...
SELECT *
FROM Subjects
WHERE subject_id IN (SELECT subject_id FROM Enrollments WHERE sdepartment_id = 3);
--6
SELECT *
FROM Departments
WHERE department_id IN (SELECT department_id FROM Subjects GROUP BY department_id HAVING COUNT(*) > 5);
--7   query will return all students who belong to the department with department_id '1'.
SELECT *
FROM Students
WHERE sdepartment_id IN (
    SELECT sdepartment_id
    FROM Departments
...
SELECT *
FROM Students
WHERE student_id not IN (SELECT student_id FROM Enrollments WHERE subject_id = 1 AND sdepartment_id = 1);
--10
SELECT *
FROM Students
WHERE student_id not IN (SELECT student_id FROM Attendance);
...
SELECT *
FROM Students
WHERE student_id not IN (SELECT student_id FROM Fees WHERE payment_date = '2022-11-20');
--13
SELECT *
FROM Students
WHERE student_id  not IN (SELECT student_id FROM FeeDefaulter);
...
SELECT *
FROM Students
WHERE student_id IN (SELECT student_id FROM FeeDefaulter WHERE sdepartment_id = 44);
--15
SELECT *
FROM Teachers
WHERE teacher_id IN (SELECT teacher_id FROM Salary WHERE amount > 100);
--16
SELECT *
FROM Teachers
WHERE teacher_id IN (SELECT teacher_id FROM Salary WHERE payment_date ='2022-11-20');
--17
SELECT *
FROM Exams
WHERE subject_id NOT IN (
...
WHERE subject_id IN (
    SELECT subject_id
    FROM Subjects
    WHERE subject_name = 'YourSubjectName'
);
...
SELECT *
FROM Students
WHERE student_id IN (SELECT student_id FROM Grades WHERE grade BETWEEN 'lower_range' AND 'upper_range');
--24
SELECT *
FROM Students
WHERE student_id IN (SELECT student_id FROM Grades WHERE subject_id = 1 AND exam_id = 5);
--25
SELECT *
FROM Students
WHERE student_id IN (SELECT student_id FROM Grades WHERE subject_id =1 OR exam_id = 2);
--26
SELECT *
FROM Students
WHERE student_id IN (SELECT student_id FROM Grades WHERE subject_id = 1 AND grade > 4);
--27
SELECT *
FROM Students
WHERE student_id IN (SELECT student_id FROM Grades WHERE subject_id = 5 OR grade < 'D');
--28
SELECT *
FROM Students
WHERE student_id IN (SELECT student_id FROM Grades WHERE subject_id = 6) AND sdepartment_id =4;
--29
SELECT *
FROM Students
WHERE student_id IN (SELECT student_id FROM Grades WHERE subject_id = 3) OR sdepartment_id = 12;
--30
SELECT *
FROM Students
WHERE student_id IN (SELECT student_id FROM Grades WHERE subject_id = 1 AND exam_id = 1) OR sdepartment_id = 2;





--(14)Aggregate functions MAX, MIN, SUM, COUNT, and AVG. 
...
--(15)Aggregate functions using logical Operators and Group by 
...
BEGIN
    ...
    FROM Subjects
    INNER JOIN departments ON Subjects.sdepartment_id = departments.department_id;
END;
------------
...
END;
---------
CREATE PROCEDURE GetStudentsFromDepartmentWithFeeDue------------------------------------------sdsdsfds
    @department_id INT
AS
BEGIN
    SELECT Students.*
    FROM Students
    INNER JOIN Feedefaulter ON Students.student_id = Feedefaulter.student_id
    WHERE Students.sdepartment_id = @department_id
    AND Feedefaulter.amountdue> 0;
END;
--------
...
END;
-------------
CREATE PROCEDURE GetTeachersFromDepartmentWithSalaryPayment-----------------------fdfdfgdg
    @department_id INT,
    @payment_date DATE
AS
BEGIN
    ...
    INNER JOIN salary ON Teachers.teacher_id = salary.teacher_id
    WHERE Teachers.department_id = @department_id
    AND salary.payment_date = @payment_date
	end
-----------
...
---------------
CREATE PROCEDURE GetStudentsByyyDepartment
...
------------------------
CREATE PROCEDURE GetStudentCountByDepartmentAndgeender----------------------sdsd
    @department_id INT,
    @gender VARCHAR(10)
AS
BEGIN
...
--------------
CREATE PROCEDURE GetStudentCountByDepartmentAndGrades-------------gdfgdfg
    @department_id INT,
    @grade VARCHAR(10)
AS
BEGIN
...
END;
INSERT INTO SDepartments (sdepartment_id,sdepartment_name)
VALUES (1,'ENG');
...
END;
INSERT INTO Attendance (subject_id,attendance_id)
VALUES (1,1);
...
END;
INSERT INTO Fees (fee_id,sdepartment_id)
VALUES (1,1);
...
END;
INSERT INTO FeeDefaulter (feedef_id,sdepartment_id)
VALUES (1,1);
...
END;
INSERT INTO Salary (salary_id,department_id)
VALUES (1,1);
...
END;
INSERT INTO Exams (exam_id,subject_id)
VALUES (2,1);
...
END;
INSERT INTO Grades (grade_id,exam_id,grade)
VALUES (2,1,'A');
...
-----------------------------------------------------
----(25)DML Triggers update 

CREATE TRIGGER TR_Students_FORupdate
ON Students
AFTER UPDATE
AS
BEGIN
    ...
    INSERT INTO StudentAudit 
    VALUES (
        'Updated student with id=' + CAST(@student_id AS VARCHAR(10)) +
        '&name=' + @first_name +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
END;

CREATE TRIGGER TR_Subjects_FORupdate
ON Subjects
AFTER UPDATE
AS
BEGIN
    ...
    INSERT INTO SubjectAudit
    VALUES (
        'Updated subject with id=' + CAST(@subject_id AS VARCHAR(10)) +
        '&name=' + @subject_name +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
END;

CREATE TRIGGER TR_Departments_FORupdate
ON Departments
AFTER UPDATE
AS
BEGIN
    ...
    INSERT INTO DepartmentAudit 
    VALUES (
        'Updated department with id=' + CAST(@department_id AS VARCHAR(10)) +
        '&name=' + @department_name +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
END;

CREATE TRIGGER TR_StudentDepartments_FORupdate
ON SDepartments
AFTER UPDATE
AS
BEGIN
    ...
    INSERT INTO StudentDepartmentAudit 
    VALUES (
        'Updated student department with id=' + CAST(@sdepartment_id AS VARCHAR(10)) +
        '&name=' + @sdepartment_name +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
END;
...
CREATE TRIGGER TR_Teachers_FORupdate
ON Teachers
AFTER UPDATE
AS
BEGIN
    ...
    INSERT INTO TeacherAudit 
    VALUES (
        'Updated teacher with id=' + CAST(@teacher_id AS VARCHAR(10)) +
        '&name=' + @first_name +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
END;
...
CREATE TRIGGER TR_Enrollments_FORupdate
ON Enrollments
AFTER UPDATE
AS
BEGIN
    ...
    INSERT INTO EnrollmentAudit
    VALUES (
        'Updated enrollment with id=' + CAST(@enrollment_id AS VARCHAR(10)) +
        '&student_id=' + CAST(@student_id AS VARCHAR(10)) +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
END;
...
CREATE TRIGGER TR_Attendance_FORupdate
ON Attendance
AFTER UPDATE
AS
BEGIN
    ...
    INSERT INTO AttendanceAudit 
    VALUES (
        'Updated attendance record with id=' + CAST(@attendance_id AS VARCHAR(10)) +
        '&student_id=' + CAST(@student_id AS VARCHAR(10)) +
        '&subject_id=' + CAST(@subject_id AS VARCHAR(10)) +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
...
CREATE TRIGGER TR_Fees_FORupdate
ON Fees
AFTER UPDATE
AS
BEGIN
    ...
    INSERT INTO FeesAudit 
    VALUES (
        'Updated fee record with id=' + CAST(@fee_id AS VARCHAR(10)) +
        '&student_id=' + CAST(@student_id AS VARCHAR(10)) +
        '&sdepartment_id=' + CAST(@sdepartment_id AS VARCHAR(10)) +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
...
CREATE TRIGGER TR_FeeDefaulter_FORupdate
ON FeeDefaulter
AFTER UPDATE
AS
BEGIN
    ...
    INSERT INTO FeeDefaulterAudit 
    VALUES (
        'Updated fee defaulter record with id=' + CAST(@feedef_id AS VARCHAR(10)) +
        '&student_id=' + CAST(@student_id AS VARCHAR(10)) +
        '&sdepartment_id=' + CAST(@sdepartment_id AS VARCHAR(10)) +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
...
CREATE TRIGGER TR_Salary_FORupdate
ON Salary
AFTER UPDATE
AS
BEGIN
    ...
    INSERT INTO SalaryAudit 
    VALUES (
        'Updated salary record with id=' + CAST(@salary_id AS VARCHAR(10)) +
        '&teacher_id=' + CAST(@teacher_id AS VARCHAR(10)) +
        '&department_id=' + CAST(@department_id AS VARCHAR(10)) +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
...
BEGIN
    ...
    INSERT INTO ExamsAudit 
    VALUES (
        'Updated exam record with id=' + CAST(@exam_id AS VARCHAR(10)) +
        '&subject_id=' + CAST(@subject_id AS VARCHAR(10)) +
        '&exam_date=' + CAST(@exam_date AS VARCHAR(20)) +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
...
CREATE TRIGGER TR_Grades_FORupdate
ON Grades
AFTER UPDATE
AS
BEGIN
    ...
    INSERT INTO GradesAudit 
    VALUES (
        'Updated grade record with id=' + CAST(@grade_id AS VARCHAR(10)) +
        '&student_id=' + CAST(@student_id AS VARCHAR(10)) +
        '&subject_id=' + CAST(@subject_id AS VARCHAR(10)) +
        '&exam_id=' + CAST(@exam_id AS VARCHAR(10)) +
        '&grade=' + @grade +
...
BEGIN
    INSERT INTO AttendanceAudit 
    VALUES ('Attendance record updated: ' + CAST(GETDATE() AS VARCHAR(20)));
END;

CREATE TRIGGER Fees_FORupdate
ON Fees
AFTER update
AS
BEGIN
    INSERT INTO FeesAudit 
    VALUES ('Fee record updated: ' + CAST(GETDATE() AS VARCHAR(20)));
END;


CREATE TRIGGER FeeDefaulter_FORupdate
ON FeeDefaulter
AFTER update
AS
BEGIN
    INSERT INTO FeeDefaulterAudit 
    VALUES ('fee defaulter record updated ' + CAST(GETDATE() AS VARCHAR(20)));
END;
CREATE TRIGGER Salary_FORupdate
ON Salary
AFTER update
AS
BEGIN
    INSERT INTO SalaryAudit 
    VALUES (' salary record updated ' + CAST(GETDATE() AS VARCHAR(20)));
END;
CREATE TRIGGER Exams_FORupdate
ON Exams
AFTER update
AS
BEGIN
    INSERT INTO ExamsAudit 
    VALUES ('updated exam scheduled at ' + CAST(GETDATE() AS VARCHAR(20)));
END;
CREATE TRIGGER Grades_FORupdate
ON Grades
AFTER update
AS
BEGIN
    INSERT INTO GradesAudit 
    VALUES ('updated grade recorded at ' + CAST(GETDATE() AS VARCHAR(20)));
END;
CREATE TRIGGER Departments_FORupdated
ON Departments
AFTER update
AS
BEGIN
    INSERT INTO DepartmentAudit 
    VALUES ('Department record updated:  ' + CAST(GETDATE() AS VARCHAR(20)));
END;
CREATE TRIGGER Teachers_FORupdate
ON Teachers
AFTER update
AS
BEGIN
    INSERT INTO TeacherAudit 
    VALUES (' teacher record updated ' + CAST(GETDATE() AS VARCHAR(20)));
END;


------------------------------------------------

--(26) TR_Students_FORdelete
CREATE TRIGGER Students_fORdelete
ON Students
AFTER DELETE
AS
BEGIN
    ...
    INSERT INTO StudentAudit 
    VALUES (
        'Deleted student with id=' + CAST(@student_id AS VARCHAR(10)) +
        '&name=' + @first_name +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
END;

-- TR_Subjects_FORDELETE
CREATE TRIGGER TR_Subjects_FORDELETE
ON Subjects
AFTER DELETE
AS
BEGIN
    ...
    INSERT INTO SubjectAudit
    VALUES (
        'Deleted subject with id=' + CAST(@subject_id AS VARCHAR(10)) +
        '&name=' + @subject_name +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
END;

-- TR_Departments_FORDELETE
CREATE TRIGGER TR_Departments_FORDELETE
ON Departments
AFTER DELETE
AS
BEGIN
    ...
    INSERT INTO DepartmentAudit 
    VALUES (
        'Deleted department with id=' + CAST(@department_id AS VARCHAR(10)) +
        '&name=' + @department_name +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
END;

-- TR_StudentDepartments_FORDELETE
CREATE TRIGGER TR_StudentDepartments_FORDELETE
ON SDepartments
AFTER DELETE
AS
BEGIN
    ...
    INSERT INTO StudentDepartmentAudit 
    VALUES (
        'Deleted student department with id=' + CAST(@sdepartment_id AS VARCHAR(10)) +
        '&name=' + @sdepartment_name +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
END;

-- TR_Teachers_FORDELETE
CREATE TRIGGER TR_Teachers_FORDELETE
ON Teachers
AFTER DELETE
AS
BEGIN
    ...
    INSERT INTO TeacherAudit 
    VALUES (
        'Deleted teacher with id=' + CAST(@teacher_id AS VARCHAR(10)) +
        '&name=' + @first_name +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
END;

-- TR_Enrollments_FORDELETE
CREATE TRIGGER TR_Enrollments_FORDELETE
ON Enrollments
AFTER DELETE
AS
BEGIN
    ...
    INSERT INTO EnrollmentAudit
    VALUES (
        'Deleted enrollment with id=' + CAST(@enrollment_id AS VARCHAR(10)) +
        '&student_id=' + CAST(@student_id AS VARCHAR(10)) +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
END;

-- TR_Attendance_FORDELETE
CREATE TRIGGER TR_Attendance_FORDELETE
ON Attendance
AFTER DELETE
AS
BEGIN
    ...
    INSERT INTO AttendanceAudit 
    VALUES (
        'Deleted attendance record with id=' + CAST(@attendance_id AS VARCHAR(10)) +
        '&student_id=' + CAST(@student_id AS VARCHAR(10)) +
        '&subject_id=' + CAST(@subject_id AS VARCHAR(10)) +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
...
-- TR_Fees_FORDELETE
CREATE TRIGGER TR_Fees_FORDELETE
ON Fees
AFTER DELETE
AS
BEGIN
    ...
    INSERT INTO FeesAudit 
    VALUES (
        'Deleted fee record with id=' + CAST(@fee_id AS VARCHAR(10)) +
        '&student_id=' + CAST(@student_id AS VARCHAR(10)) +
        '&sdepartment_id=' + CAST(@sdepartment_id AS VARCHAR(10)) +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
...
-- TR_FeeDefaulter_FORDELETE
CREATE TRIGGER TR_FeeDefaulter_FORDELETE
ON FeeDefaulter
AFTER DELETE
AS
BEGIN
    ...
    INSERT INTO FeeDefaulterAudit 
    VALUES (
        'Deleted fee defaulter record with id=' + CAST(@feedef_id AS VARCHAR(10)) +
        '&student_id=' + CAST(@student_id AS VARCHAR(10)) +
        '&sdepartment_id=' + CAST(@sdepartment_id AS VARCHAR(10)) +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
...
BEGIN
    ...
    INSERT INTO SalaryAudit 
    VALUES (
        'Deleted salary record with id=' + CAST(@salary_id AS VARCHAR(10)) +
        '&teacher_id=' + CAST(@teacher_id AS VARCHAR(10)) +
        '&department_id=' + CAST(@department_id AS VARCHAR(10)) +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
...
BEGIN
    ...
    INSERT INTO ExamsAudit 
    VALUES (
        'Deleted exam record with id=' + CAST(@exam_id AS VARCHAR(10)) +
        '&subject_id=' + CAST(@subject_id AS VARCHAR(10)) +
        '&exam_date=' + CAST(@exam_date AS VARCHAR(20)) +
        ' at ' + CAST(GETDATE() AS VARCHAR(20))
    );
...
-- TR_Grades_FORDELETE
CREATE TRIGGER TR_Grades_FORDELETE
ON Grades
AFTER DELETE
AS
BEGIN
    ...
    INSERT INTO GradesAudit 
    VALUES (
        'Deleted grade record with id=' + CAST(@grade_id AS VARCHAR(10)) +
        '&student_id=' + CAST(@student_id AS VARCHAR(10)) +
        '&subject_id=' + CAST(@subject_id AS VARCHAR(10)) +
        '&exam_id=' + CAST(@exam_id AS VARCHAR(10)) +
        '&grade=' + @grade +
...
END;
CREATE TRIGGER Trigger_for_del
ON Attendance
AFTER delete
AS
BEGIN
    INSERT INTO AttendanceAudit 
    VALUES ('Deleted attendance record at ' + CAST(GETDATE() AS VARCHAR(20)));
END;


CREATE TRIGGER FeesFORdelete
ON Fees
AFTER delete
AS
BEGIN
    INSERT INTO FeesAudit 
    VALUES ('deleteed fee record is  ' + CAST(GETDATE() AS VARCHAR(20)));
END;
CREATE TRIGGER FeeDefaulter_FORdelete
ON FeeDefaulter
AFTER delete
AS
BEGIN
    INSERT INTO FeeDefaulterAudit 
    VALUES ('deleted fee defaulter record is ' + CAST(GETDATE() AS VARCHAR(20)));
END;
CREATE TRIGGER Salary_FORdelete
ON Salary
AFTER delete
AS
BEGIN
    INSERT INTO SalaryAudit 
    VALUES ('deleated salary  is  ' + CAST(GETDATE() AS VARCHAR(20)));
END;
CREATE TRIGGER Exams_FORdelete
ON Exams
AFTER delete
AS
BEGIN
    INSERT INTO ExamsAudit 
    VALUES ('deleated exam scheduled is ' + CAST(GETDATE() AS VARCHAR(20)));
END;
CREATE TRIGGER Grades_FORdeleate
ON Grades
AFTER delete
AS
BEGIN
    INSERT INTO GradesAudit 
    VALUES ('delete grade recorded is ' + CAST(GETDATE() AS VARCHAR(20)));
END;
CREATE TRIGGER Departments_FORdelete
ON Departments
AFTER delete
AS
BEGIN
    INSERT INTO DepartmentAudit 
    VALUES ('delete department is ' + CAST(GETDATE() AS VARCHAR(20)));
END;
CREATE TRIGGER Teachers_FORdelete
ON Teachers
AFTER delete
AS
BEGIN
    INSERT INTO TeacherAudit 
    VALUES (' delete teacher  is' + CAST(GETDATE() AS VARCHAR(20)));
END;


---------

---(29)Single-Row Functions UPPER, LOWER, LENGTH, SUBSTR using logical operators
...
---(30)-Single-Row Functions TRIM, REPLACE, ROUND, TRUNC using logical operators 

SELECT TRIM(Aaddress) AS trimmed_Aaddress
FROM Students;
SELECT CAST(amount AS decimal(10, 2)) AS truncated_amount
FROM Fees;
SELECT ROUND(credithour, 2) AS rounded_credit
FROM Subjects;
SELECT TRIM(first_name) AS trimmed_first_name, TRIM(last_name) AS trimmed_last_name
FROM Students;
SELECT REPLACE(first_name, 'Mr.', 'Ms.') AS replaced_first_name
FROM Students;
SELECT first_name, last_name, CAST(date_of_birth AS DATE) AS truncated_date_of_birth
FROM Students;
UPDATE departments
SET department_name = REPLACE(department_name, 'Engineering', 'Computer Science')
WHERE department_name = 'Engineering';
SELECT student_id, ROUND(amount,2) AS rounded_amount
FROM Fees;
SELECT student_id, CAST(payment_date AS DATE) AS truncated_payment_date
FROM Fees;
UPDATE Students
SET Aaddress = REPLACE(Aaddress, 'USA', 'United States');
SELECT ROUND(AVG(credithour), 2) AS rounded_average_credit_hour
FROM Subjects;
UPDATE departments
SET department_name = 'Business Administration'
WHERE department_name = 'Management';
SELECT student_id, ROUND(amountdue, -1) AS rounded_amountdue
FROM FeeDefaulter;
SELECT subject_id, CAST(DATEPART(YEAR, exam_date) AS varchar(max)) AS truncated_exam_year
FROM exams;
UPDATE Teachers
SET last_name = 'Doe'
WHERE first_name LIKE '%A%';
UPDATE Subjects
SET subject_name = REPLACE(subject_name, 'Physics', 'Chemistry')
WHERE subject_name = 'Physics';
SELECT ROUND(AVG(DATEDIFF(year, date_of_birth, GETDATE())), 0) AS rounded_average_age
FROM Students;
UPDATE Students
SET gender = CASE
    ...
    ELSE gender
  END;
SELECT department_id, CAST(department_id AS decimal(10, 2)) AS truncated_department_id
FROM departments;
UPDATE Subjects
...
FROM Fees;
SELECT student_id, DATEFROMPARTS(YEAR(payment_date), MONTH(payment_date), 1) AS truncated_payment_month
FROM Fees;
UPDATE Teachers
SET last_name = 'Johnson'
WHERE first_name LIKE 'D%' OR first_name LIKE '%s';
UPDATE Students
SET Aaddress = REPLACE(Aaddress, '123', '456');
SELECT first_name, last_name, CAST(LEFT(contact_number, 3) AS VARCHAR) AS truncated_contact_number
FROM Students;
UPDATE departments
SET department_name = 'Accounting'
WHERE department_name = 'Finance';
SELECT student_id, ROUND(amountdue, -2) AS rounded_amountdue
FROM FeeDefaulter;
SELECT subject_id, CAST(CONCAT(YEAR(exam_date), '-', MONTH(exam_date), '-01') AS DATE) AS truncated_exam_month
FROM exams;
UPDATE Teachers
SET first_name = 'Emily'
WHERE last_name LIKE '%E%';
UPDATE Subjects
SET subject_name = REPLACE(subject_name, 'Chemistry', 'Physics')
WHERE subject_name = 'Chemistry';

SELECT ROUND(AVG(DATEDIFF(year, date_of_birth, GETDATE())) / 5,5) * 5 AS rounded_average_age
FROM Students;
SELECT ROUND(AVG(credithour) /2, 5) * 5 AS rounded_average_credit_hour
FROM Subjects;
UPDATE Teachers
SET gender = CASE
    WHEN gender = 'Male' THEN 'Female'
    ...
    ELSE gender
  END;
SELECT TRIM(subject_name) AS trimmed_subject_name
FROM Subjects;
SELECT s.student_id, REPLACE(s.Aaddress, 'Department', 'Department of') AS updated_address
FROM Students s
JOIN departments d ON s.sdepartment_id = d.department_id;
SELECT TRIM(LOWER(subject_name)) AS trimmed_lower_subject_name
FROM Subjects;
SELECT Teacher_id, REPLACE(REPLACE(contact_number, '-', ''), ' ', '') AS cleaned_contact_number
FROM Teachers;
SELECT student_id, first_name, last_name, ROUND(DATEDIFF(YEAR, date_of_birth, GETDATE()), 0) AS rounded_age
FROM Students;
SELECT subject_id, subject_name, CAST(ROUND(credithour, 2) AS decimal(10, 2)) AS truncated_credit_hour
FROM Subjects;
SELECT student_id, REPLACE(email,first_name ,1) AS updated_email
FROM Students;
SELECT subject_id, CAST(LEFT(subject_name, 3) AS VARCHAR(3)) AS truncated_subject_name
FROM Subjects;
SELECT student_id, first_name, last_name, CAST(MONTH(date_of_birth) AS INT) AS truncated_birth_month
FROM Students;
SELECT student_id, UPPER(TRIM(first_name)) AS trimmed_uppercase_first_name
FROM Students;
SELECT  first_name, last_name, ROUND(amount, -3) AS rounded_salary_amount
FROM Teachers,salary;
SELECT *
FROM Teachers
WHERE CHARINDEX('s', last_name) > 0 AND TRIM(contact_number) = contact_number;
SELECT REPLACE(last_name, ' ', '_') AS updated_last_name, CAST(FLOOR(YEAR(date_of_birth) / 10.0) * 10 AS INT) AS truncated_birth_date
FROM Students;
SELECT teacher_id, REPLACE(TRIM(first_name), ' ', CONCAT(' - ', department_id, ' - ')) AS updated_first_name
FROM Teachers;
SELECT CONCAT(REPLACE(last_name, ' ', ' - '), ' - ', TRIM(contact_number)) AS updated_info
FROM Students;
SELECT REPLACE(subject_name, ' ', CONCAT(' - ', Subject_id, ' - ')) AS updated_subject_name, ROUND(credithour, 2) AS rounded_credit_hour
FROM Subjects;
SELECT TRIM(teachers.first_name) AS trimmed_first_name, ROUND(salary.amount, -3) AS rounded_salary_amount
FROM Teachers teachers, salary;




-------------------(32)-Exception handling - Try Catch 
BEGIN TRY
